...
class WifiState(MacState):
    ...
    def estimate_station_position(self, measurements: list[tuple[float, float, int]]) -> tuple[float, float, float, float, float]:
        ...
        ap.estimate_with_fixed_path_loss(-44, 2.7)
        return ap.lat, ap.lon, ap.A, ap.n, ap.trust

    def estimate_station_as_average(self, measurements: list[tuple[float, float, int]]) -> tuple[float, float, float, float, float]:
        weights = numpy.array(
            [obs.weight for obs in self.observations], dtype=numpy.double
        )

        coords = numpy.array(
            [(lat, lon) for lat, lon, _ in measurements],
            dtype=numpy.double
        )

        lat, lon = numpy.average(coords, axis=0, weights=weights)
        lat = float(lat)
        lon = float(lon)

        _, weight = self.bounded_samples_weight(
            len(self.observations), float(weights.sum())
        )
        return lat, lon, -45, 2.7, weight / 4
    ...
    def aggregate_station_obs(self):
        ...
        # else:
        #     lat, lon, A, n, _ = self.estimate_station_as_average(measurements)
